...
function cleanup
{
	...
	rm_process
	safeecho $color_reset
	wipe
	echo -n "$showcursor"
	stty echo
	! $saved_term || tput rmcup
	case "$uname" in
		Darwin)
			reset
		;;
...
# hide
hidecursor=$(tput civis 2>/dev/null)	# hide cursor
showcursor=$(tput cnorm 2>/dev/null)	# show cursor
		...
		case "$state_current" in
			...
			ingame)
				...
				if $pause_last; then # this uses pause_last to avoid drawing the pause blinker before the frame
					...
					# sleep a fifth of a second to save cpu (lowers framerate drastically while pausing)
					sleep 0.2
				fi
